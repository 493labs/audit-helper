...
def test_overflow_check():
    solidity_contract = get_contract(None, './tests/bytecode/examples/overflow.sol','Overflow')
    assert overflow_check(solidity_contract.disassembly) == True

def test_no_overflow_check():
    solidity_contract = get_contract(None, './tests/bytecode/examples/overflow.sol','NoOverflow')
